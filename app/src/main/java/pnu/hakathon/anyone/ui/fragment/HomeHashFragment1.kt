...
class HomeHashFragment1 : Fragment() {
    ...
    override fun onCreateView(
        ...
        mainViewModel.store_quiet.observe(viewLifecycleOwner, Observer {
            adapter.setList(it)
        ...
        mainViewModel.isLoading.observe(viewLifecycleOwner, Observer {
            it?.let {
                if (it) {
                    v.hash_list_shimmer.startShimmer()
                } else {
                    v.hash_list_shimmer.stopShimmer()
                }
                v.hash_list_shimmer.visibility = if (it) View.VISIBLE else View.GONE
            }
        })
