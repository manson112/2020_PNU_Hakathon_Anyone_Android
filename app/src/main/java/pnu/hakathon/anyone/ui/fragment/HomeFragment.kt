...
import pnu.hakathon.anyone.ui.adapter.TabAdapter
...
class HomeFragment : Fragment(){
    ...
    override fun onCreateView(
        ...
        v.home_hash_pager.isUserInputEnabled = false

        v.home_searchbar.setOnClickListener {
            startActivity(Intent(requireActivity(), SearchActivity::class.java).putExtra("categoryID", mainViewModel.categoryID))
        }

        val adapter =
            HomeFragmentListAdapter(container.context)
        v.home_recommend_recyclerview.adapter = adapter
        val itemDecorator = DividerItemDecoration(context, DividerItemDecoration.VERTICAL)
        ContextCompat.getDrawable(container.context, R.drawable.divider_home_list)?.let {
