...
class HomeHashFragment2 : Fragment() {
    ...
    override fun onCreateView(
        ...
        context.homeViewModel.list2.list1.observe(context, Observer {
            adapter1.setList(it)
        })
        context.homeViewModel.list2.list2.observe(context, Observer {
            adapter2.setList(it)
        })
        context.homeViewModel.list2.list3.observe(context, Observer {
            adapter3.setList(it)
        })
        context.homeViewModel.setB()
